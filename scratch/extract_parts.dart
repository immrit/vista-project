...
void main() {
  final file = File(r'E:\vista\lib\features\chat\screens\modern_chat_screen.dart');
  final content = file.readAsStringSync();

  // Find start index of each section
  final appBarStart = content.indexOf('  Widget _buildEmojiPanel(ChatTheme theme) {');
  final listStart = content.indexOf('  Widget _buildMessageList(');
  final inputStart = content.indexOf('  Widget _buildInputDockHalo({');
  final bubblesStart = content.indexOf('  Widget _buildMessagePreviewWidget(MessageModel message, bool isMe) {');
  ...
  if (appBarStart == -1 || listStart == -1 || inputStart == -1 || bubblesStart == -1 || endOfStateClass == -1) {
  ...
  final bubblesCode = content.substring(bubblesStart, endOfStateClass - 2); // -2 for trailing \n}

  // Write to part files
  _writePartFile('modern_chat_screen_app_bar.dart', 'ModernChatAppBarExt', appBarCode);
  _writePartFile('modern_chat_screen_list.dart', 'ModernChatListExt', listCode);
  _writePartFile('modern_chat_screen_input.dart', 'ModernChatInputExt', inputCode);
  _writePartFile('modern_chat_screen_bubbles.dart', 'ModernChatBubblesExt', bubblesCode);

  // Update main file
  final mainCode = content.substring(0, appBarStart) + '}\n\n' + content.substring(endOfStateClass);
  ...
  final newMainCode = mainCode.substring(0, endOfLastImport) + 
