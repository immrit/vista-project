...
void main() {
  ...
  for (final file in files) {
    ...
    if (content.contains('catch (_) {}')) {
      content = content.replaceAll('catch (_) {}',
    ...
    if (file.path.endsWith('chat_repository_impl.dart')) {
      final m1 = RegExp(
          r'final Map<String, int> _lastMsgSyncMs = \{\};\s*final Map<String, bool> _msgSyncInFlight = \{\};\s*int _lastConvSyncMs = 0;\s*bool _convSyncInFlight = false;\s*int _convRateLimitedUntilMs = 0;');
      if (m1.hasMatch(content)) {
        content = content.replaceFirst(m1,
      ...
      if (m2.hasMatch(content)) {
        content = content.replaceFirst(m2,
      ...
      final m3 = RegExp(
          r'Future<void> _syncConversations\(String uid\) async \{\s*if \(_convSyncInFlight\) return;\s*final now = DateTime\.now\(\)\.millisecondsSinceEpoch;\s*if \(now < _convRateLimitedUntilMs\) return;\s*if \(now - _lastConvSyncMs < _convSyncThrottle\.inMilliseconds\) return;\s*_lastConvSyncMs = now;\s*_convSyncInFlight = true;');
      if (m3.hasMatch(content)) {
        content = content.replaceFirst(m3,
      ...
      final m6 = RegExp(
          r'\} finally \{\s*_msgSyncInFlight\.remove\(conversationId\);\s*\}\s*\}');
      if (m6.hasMatch(content)) {
        content = content.replaceFirst(m6,
