...
- [x] **P3** — **batched feed analytics** + **routed `GoPostsRepository` through the shared pinned client** (`createApiV1Dio` → cert pinning + god-mode interceptors: maintenance/ban/rate-limit/feature-disabled) instead of a bare `Dio`. §3.4 / §8.4 *(remainder: route the other ~25 bare-Dio services the same way + add a central 401-refresh/retry interceptor — mechanical, see MEMORY.md.)*
- [~] **P4** — provider hygiene: started — `postProvider` (per-postId detail) → `autoDispose` (clearest leak). §3.2 / §8.5 *(blanket pass over ~370 providers still staged — risks state-loss regressions invisible to `flutter test`; safe approach in MEMORY.md.)*
