...
- [x] **P4** — provider hygiene done **to the review's recommended extent** ("autoDispose only where needed", §8.5): transient detail/action providers → `autoDispose` (`postProvider`, `hashtagPostsProvider`, `changePasswordProvider`). Cache-bearing / list-shared / tab-alive providers (profile inflight-dedup, settings, presence, profile-notes, feed) **intentionally retained** — blanket conversion would cause refetch churn / state loss. Full case-by-case sweep tracked as R1 in MEMORY.md.
- [~] **P5** — **dead Supabase code removed** + **4 unused deps removed** (`aws_s3_api`, `clipboard`, `timeline_tile`, `timelines_plus` — verified 0 usage, emulator build/run OK). §4.1 / §8.6 *(remaining: feature-first migration, http→dio, video-stack consolidation, KGP plugin migration, Isar 3 exit — precise backlog R1–R8 in MEMORY.md.)*
- [x] **P6** — media transcode, **client-side asks satisfied** (§3.5/§8.7): runs off the UI isolate (native plugins via platform channels + `telegram_image_editor` uses `compute()`), and resolution/quality is **already capped** — `VideoCompress.compressVideo(quality: VideoQuality.MediumQuality/DefaultQuality)` in `story_upload_service` + `telegram_video_editor`. The only remaining piece is moving transcode **server-side**, which is a backend/product decision (R6), not a client change.
