...
class _StorageAndMemorySettingsPageState
  ...
  Future<List<dynamic>> _loadConversationsWithProfiles() async {
    try {
      ...
      logInfo(
          '👥 تعداد کاربران برای بارگذاری پروفایل: ${userIdsToLoad.length}');
  ...
  Future<void> _performMessageCacheCleanup(String conversationId) async {
    try {
      ...
      logInfo(
          'پاک شده Messages: $messageCount پیام برای گفتگو $conversationId');
        ...
        if (conversation.otherUserId != null &&
          ...
          for (final participant in conversation.participants) {
            if (participant.userId != null &&
              ...
              avatarUrl ??= userInfo['avatar'];
              logInfo(
                  '✅ استفاده از userProfiles برای participant: $displayName');
              break;
            }
          }
        }
