  ...
  Future<void> _resetPassword() async {
    ...
    while (attempt < maxRetries) {
      try {
        attempt++;
        logInfo(
            '📧 تلاش $attempt/$maxRetries: ارسال کد بازیابی به ایمیل: ${_emailController.text.trim()}');
        ...
        await supabase.auth
          ...
          onTimeout: () {
            throw TimeoutException(
                'درخواست ارسال ایمیل timeout شد. لطفاً دوباره تلاش کنید');
          },
        );
        ...
        if (attempt >= maxRetries || !isRetryable) {
          ...
          if (errorString.contains('timeout') || errorString.contains('504')) {
            errorMessage =
                'ارسال ایمیل timeout شد. لطفاً اتصال اینترنت خود را بررسی کنید و دوباره تلاش کنید';
