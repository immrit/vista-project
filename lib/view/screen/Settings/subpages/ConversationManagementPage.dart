...
class ConversationManagementPage extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<ConversationManagementPage> createState() => _ConversationManagementPageState();
}

class _ConversationManagementPageState extends ConsumerState<ConversationManagementPage> {
  ...
  Future<void> _deleteSelectedConversations() async {
    ...
    final confirmed = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Text('آیا مطمئن هستید که می‌خواهید ${selectedConversations.length} مکالمه را حذف کنید؟'),
        actions: [
          TextButton(
    ...
    if (confirmed == true) {
      try {
        ...
        for (final conversationId in selectedConversations) {
        ...
        content: SizedBox(
          ...
          child: Column(
            ...
            children: [
              ...
              Text('حجم تقریبی: ${_formatSize(conversationSizes[conversation.id] ?? 0)}'),
              Text('آخرین فعالیت: ${_formatDate(conversation.lastMessageTime ?? conversation.updatedAt)}'),
              if (conversation.lastMessage != null)
                Text('آخرین پیام: ${conversation.lastMessage}', 
                     maxLines: 2, overflow: TextOverflow.ellipsis),
              const SizedBox(height: 16),
              if (messages.isNotEmpty) ...[
                const Text('آخرین پیام‌ها:', style: TextStyle(fontWeight: FontWeight.bold)),
  ...
  Widget build(BuildContext context) {
    ...
    final totalSize = conversationSizes.values.fold(0.0, (sum, size) => sum + size);
    
    return Scaffold(
      appBar: AppBar(
        title: Text(isSelectMode ? '${selectedConversations.length} انتخاب شده' : 'مدیریت مکالمات'),
        ...
        actions: [
          if (!isSelectMode) ...[
            IconButton(
              icon: Icon(showSearchBar ? Icons.search_off_rounded : Icons.search_rounded),
            ...
            PopupMenuButton<String>(
              ...
              itemBuilder: (context) => [
                PopupMenuItem(value: 'date', child: Row(
                  children: [
                ...
                PopupMenuItem(value: 'size', child: Row(
                  children: [
                ...
                PopupMenuItem(value: 'messages', child: Row(
                  children: [
            ...
            TextButton(
              onPressed: selectedConversations.length == conversations.length ? _deselectAll : _selectAll,
              child: Text(selectedConversations.length == conversations.length ? 'لغو همه' : 'انتخاب همه'),
            ),
            IconButton(
              icon: const Icon(Icons.delete_rounded),
              onPressed: selectedConversations.isEmpty ? null : _deleteSelectedConversations,
              color: Colors.red,
          ...
          Container(
            ...
            child: Row(
              ...
              children: [
                ...
                Column(
                  children: [
                    Text(conversationMessages.values.fold(0, (sum, msgs) => sum + msgs.length).toString(),
                    ...
                    ? const Center(
                        child: Column(
                          ...
                          children: [
                            ...
                            SizedBox(height: 16),
                            Text('هیچ مکالمه‌ای یافت نشد', style: TextStyle(color: Colors.grey, fontSize: 16)),
                          ],
                        ),
                      )
                    : ListView.builder(
                        ...
                        itemBuilder: (context, index) {
                          final filteredConversations = _getFilteredAndSortedConversations();
                          final conversation = filteredConversations[index];
                          final isSelected = selectedConversations.contains(conversation.id);
                          final messageCount = conversationMessages[conversation.id]?.length ?? 0;
                          ...
                          return Card(
                            ...
                            child: ListTile(
                              leading: isSelectMode
                                  ? Checkbox(
                                      value: isSelected,
                                      onChanged: (_) => _toggleConversationSelection(conversation.id),
                                    )
                                  : CircleAvatar(
                            ...
                            title: Text(
                              ...
                              subtitle: Column(
                                ...
                                children: [
                                  ...
                                  Row(
                                    children: [
                                      Icon(Icons.message_rounded, size: 12, color: Colors.grey[600]),
                                      const SizedBox(width: 4),
                                      Text('$messageCount پیام', style: TextStyle(color: Colors.grey[600], fontSize: 12)),
                                      const SizedBox(width: 12),
                                      Icon(Icons.storage_rounded, size: 12, color: Colors.grey[600]),
                                      const SizedBox(width: 4),
                                      Text(_formatSize(size), style: TextStyle(color: Colors.grey[600], fontSize: 12)),
                                    ],
                                  ),
                                ],
                              ),
                              trailing: isSelectMode
                                  ...
                                  : Column(
                                      ...
                                      children: [
                                        Text(_formatDate(conversation.lastMessageTime ?? conversation.updatedAt),
      ...
      floatingActionButton: !isSelectMode && conversations.isNotEmpty ? FloatingActionButton.extended(
    ...
    showModalBottomSheet(
      ...
      builder: (context) => Container(
        ...
        child: Column(
          mainAxisSize: MainAxisSize.min,
          children: [
            const Text('عملیات سریع', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
            const SizedBox(height: 16),
            
            ListTile(
              leading: const Icon(Icons.auto_delete_rounded, color: Colors.blue),
            ...
            ListTile(
              leading: const Icon(Icons.select_all_rounded, color: Colors.orange),
  ...
  Future<void> _deleteOldConversations(int daysOld) async {
    ...
    final confirmed = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        title: const Text('حذف مکالمات قدیمی'),
        content: Text('${oldConversations.length} مکالمه قدیمی‌تر از $daysOld روز یافت شد. آیا می‌خواهید آن‌ها را حذف کنید؟'),
        actions: [
  ...
  Future<void> _deleteEmptyConversations() async {
    ...
    final confirmed = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        title: const Text('حذف مکالمات خالی'),
        content: Text('${emptyConversations.length} مکالمه خالی یافت شد. آیا می‌خواهید آن‌ها را حذف کنید؟'),
        actions: [
          TextButton(onPressed: () => Navigator.pop(context, false), child: const Text('لغو')),
          TextButton(
            onPressed: () => Navigator.pop(context, true),
  ...
  Future<void> _deleteLargeConversations(double sizeMB) async {
    ...
    if (largeConversations.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('هیچ مکالمه بزرگ‌تر از ${sizeMB.toStringAsFixed(0)}MB یافت نشد')),
      );
      return;
    }
    
    final totalSize = largeConversations.fold(0.0, (sum, conv) => sum + (conversationSizes[conv.id] ?? 0));
    
    final confirmed = await showDialog<bool>(
      context: context,
      builder: (context) => AlertDialog(
        title: const Text('حذف مکالمات حجیم'),
        content: Text('${largeConversations.length} مکالمه بزرگ‌تر از ${sizeMB.toStringAsFixed(0)}MB یافت شد (مجموع ${_formatSize(totalSize)}). آیا می‌خواهید آن‌ها را حذف کنید؟'),
        actions: [
          TextButton(onPressed: () => Navigator.pop(context, false), child: const Text('لغو')),
          TextButton(
            onPressed: () => Navigator.pop(context, true),
  ...
  void _selectOldConversations(int daysOld) {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(content: Text('${selectedConversations.length} مکالمه قدیمی انتخاب شد')),
    );
  }
  ...
  void _selectLowMessageConversations(int maxMessages) {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(content: Text('${selectedConversations.length} مکالمه کم‌پیام انتخاب شد')),
    );
  }
  ...
  void _selectLargeConversations(double sizeMB) {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(content: Text('${selectedConversations.length} مکالمه حجیم انتخاب شد')),
    );
  }
}
